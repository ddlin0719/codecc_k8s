...
cp -rf gateway/codecc_render_tpl tmp/
...
rm -rf tmp/*
cp -rf bkci/support-files tmp/
cp -rf configuration/import_config_codecc.sh tmp/
cp -rf configuration/codecc_render_tpl tmp/
docker build -f configuration/configuration_codecc.Dockerfile -t $hub/bkci-configuration:1.0 tmp --network=host
